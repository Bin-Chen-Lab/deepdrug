...
parser.add_argument('--beta_d', type=float, default=0.5, help='beta D for adam. default=0.5')
parser.add_argument('--beta_g', type=float, default=0.5, help='beta G for adam. default=0.5')
...
parser.add_argument('--noise_std', type=float, default=0.1, help='gausian noise std')
parser.add_argument('--sd_fold', type=int, default=1, help='times of sd')
...
# setup optimizer
optimizer_g = optim.Adam(generator.parameters(), lr=opt.lr_g, betas=(opt.beta_g, 0.9), eps=0.01)
optimizer_d = optim.Adam(dscrmntor.parameters(), lr=opt.lr_d, betas=(opt.beta_d, 0.9), eps=0.01)
...
for iter_idx in range(1, opt.n_epoch * len(data_loader_real)+1):
    ...
    if opt.add_noise:
        batch_fake_noise = gaussian(batch_fake)
    else:
        batch_fake_noise = batch_fake
        
    residual = generator(batch_fake_noise)*(opt.sd_fold * standard_deviation)

    logits_fake = dscrmntor(batch_fake_noise + residual)
    ...
    if train_d:
        ...
        if (precision_real[0] > 90 and precision_fake[0] > 90) or train_d_iter > opt.train_d:
            train_d_iter = 0
            train_d = False
    else:  # train g
        ...
        if precision_fake[0] > 90 or train_g_iter > opt.train_g:
            train_g_iter = 0
            train_d = True
