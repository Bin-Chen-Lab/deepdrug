...
dataset_real = torch.utils.data.TensorDataset(data_tensor_real, torch.Tensor(y[:split]))
data_loader_real = torch.utils.data.DataLoader(dataset_real, batch_size=opt.batch_size, shuffle=True)

dataset_fake = torch.utils.data.TensorDataset(data_tensor_fake, torch.Tensor(y[split:]))
...
batch_real_label = torch.LongTensor(opt.batch_size, data_dim)
batch_fake_label = torch.LongTensor(opt.batch_size, data_dim)
...
batch_fake_label = Variable(batch_fake_label)
...
for iter_idx in range(1, opt.n_epoch * len(data_loader_real) + 1):
    samples_fake, samples_fake_label, iter_fake = get_next_batch(iter_fake, data_loader_fake)
    batch_fake_label.data.resize_(samples_fake_label.size()).copy_(samples_fake_label)        
